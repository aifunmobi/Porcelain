...
- [ ] `grep -nE "#[0-9a-fA-F]{3,8}\b|rgba?\(" src/apps/**/*.css` returns no results except for values inside a documented allowlist comment (media mats and Terminal's ANSI palette). Every other colour comes from a `var(--…)`.
- [ ] `grep -n "box-shadow:" src/apps/**/*.css` shows only `var(--…)` or primitive-class usage — no literal shadow values.
- [ ] All 15 apps open, render fully, and are usable; nothing is clipped, overlapping or unreadable.
- [ ] Buttons across all apps press in on click; inputs across all apps render as pressed wells.
- [ ] All 15 apps render correctly in dark mode.
- [ ] No console errors when opening and interacting with each app.
- [ ] No app lost a feature: Calculator computes, Notes saves, Calendar navigates months, Clock ticks, Terminal runs its commands, Music and Video play, Camera previews, Photos browses, Browser navigates, Weather loads, Settings persists, Files browses, Trash restores, Text Editor edits.
